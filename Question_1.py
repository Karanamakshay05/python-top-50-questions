'''There is an ant on your balcony.It wants to leave the rail so sometimes it moves right
...
n=int(input())
arr=list(map(int,input().split()))#Array input must be taken with this syntax
count=0
for i in range(n):
    if(sum(arr[:i+1])==0):
        count=count+1
